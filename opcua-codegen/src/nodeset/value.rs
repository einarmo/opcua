...
pub fn render_value(
    ...
    if let Some(value) = value {
        let rendered = render_variant(&value.0, opcua_path, types)?;
        Ok(quote! {
...
pub fn render_variant(
    ...
    let inner = match &value {
        ...
        Variant::NodeId(v) => {
            let id =
                opcua_xml::schema::ua_node_set::NodeId(v.identifier.clone().unwrap_or_default());
        ...
        Variant::ExtensionObject(v) => render_extension_object(v, opcua_path, types)?,
        Variant::ListOfExtensionObject(v) => {
            let mut items = quote::quote! {};
            for it in v {
                let rendered = render_extension_object(it, opcua_path, types)?;
                items.extend(quote::quote! {
                    #rendered,
                })
            }
        ...
        Variant::Variant(v) => {
            let inner = render_variant(&v, opcua_path, types)?;
            quote::quote! {
        ...
        Variant::ListOfVariant(v) => {
            let mut items = quote::quote! {};
            for it in v {
                let inner = render_variant(&it, opcua_path, types)?;
                items.extend(quote::quote! {
                    #opcua_path::types::Variant::Variant(Box::new(#inner))
                });
            }
...
fn render_extension_object(
    ...
    let content = render_extension_object_inner(&body.data, opcua_path, types)?;
...
fn render_extension_object_inner(
    ...
    let Some(typ) = types.get(ty) else {
        return Err(CodeGenError::Other(format!("Unknown type {ty}")));
    };
    // First, we need to evaluate the type
    let type_ref = make_type_ref(typ, types).map_err(|e| CodeGenError::Other(e))?;

    // Now for rendering the type itself,
    render_complex_type(&type_ref, &data, types, opcua_path)
}
    ...
    match ty {
        TypeRef::Enum(e) => {
            ...
            if e.variants.is_empty() {
                ...
                Ok(quote! {
                    #opcua_path::types::#ident::from_bits_truncate(#val.into())
                })
            } else {
                ...
                Ok(quote! {
                    #opcua_path::types::#ident::#key_ident
                })
            }
        }
        TypeRef::Struct(e) => {
            ...
            for (name, field) in &e.fields {
                let rendered = render_field(name, field, node, types, opcua_path)?;
...
fn render_field(
    ...
    let is_primitive = is_primitive(type_name);
    let ty = types
        .get(type_name)
        .map(|t| make_type_ref(t, types))
        .transpose()
    ...
    if is_array {
        ...
        if items.is_empty() {
            ...
            for item in items {
                if is_primitive {
                    let rendered = render_primitive(item, type_name, opcua_path)?;
                    it.extend(quote! {
                        #rendered,
                    })
                } else {
                    ...
                    let rendered = render_complex_type(r, item, types, opcua_path)?;
                    it.extend(quote! {
                        #rendered,
                    })
                }
        ...
        if is_primitive {
            render_primitive(item, type_name, opcua_path)
        } else {
            let Some(r) = &ty else {
                return Err(CodeGenError::Other(format!("Type {type_name} not found")));
            };
            render_complex_type(r, item, types, opcua_path)
        }
    }
}
...
fn render_primitive(
    ...
    if ty.starts_with("ListOf") {
        ...
        for elem in node.children_with_name(field_name) {
            let rendered = render_primitive(elem, field_type, opcua_path)?;
            items.extend(quote! {
                #rendered,
            })
        }
    ...
    match ty {
        ...
        "dateTime" => {
            let ts = chrono::DateTime::parse_from_rfc3339(&data)
                .map_err(|e| CodeGenError::Other(format!("Failed to parse datetime {data}: {e}")))?
                .timestamp_micros();
            Ok(quote! {
...
fn make_type_ref<'a>(
    ty: &'a XsdFileType,
    types: &'a HashMap<String, XsdFileType>,
) -> Result<TypeRef<'a>, String> {
        ...
        XsdFileType::Complex(c) => {
            ...
            let (parent, sequence) = match &c.content {
                ...
                Some(ComplexTypeContents::Complex(ComplexContent::Restriction(e))) => {
                    ...
                    let base_type = types
                        .get(base_name)
                        .ok_or_else(|| format!("Base type of {}, {} not found", name, base_name))?;
                    let TypeRef::Struct(base_type) = make_type_ref(base_type, types)? else {
                        return Err(format!("Base type of struct {} must be a struct", name));
                    };
                    let TypeDefParticle::Sequence(s) = e.particle.as_ref().ok_or_else(|| {
