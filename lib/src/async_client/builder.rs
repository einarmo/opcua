...
use crate::client::prelude::{
    ClientConfig, ClientEndpoint, ClientUserToken, Config, ANONYMOUS_USER_TOKEN_ID,
};
...
impl ClientBuilder {
    ...
    pub fn client(self) -> Option<AsyncClient> {
        if self.is_valid() {
            Some(AsyncClient::new(self.config))
        } else {
            None
        }
    }
    ...
    pub fn application_uri<T>(mut self, application_uri: impl Into<String>) -> Self {
        self.config.application_uri = application_uri.into();
        self
    }

    /// Sets the product uri.
    pub fn product_uri<T>(mut self, product_uri: impl Into<String>) -> Self {
    ...
    /// from disk.
    pub fn private_key_path<T>(mut self, private_key_path: impl Into<PathBuf>) -> Self {
    ...
    pub fn default_endpoint<T>(mut self, endpoint_id: impl Into<String>) -> Self {
