...
use crate::{
    async_client::{retry::SessionRetryPolicy, transport::State, AsyncSecureChannel},
    client::{
        prelude::{
...
impl SessionEventLoop {
    pub async fn run(self) -> Result<(), StatusCode> {
        {
            if !matches!(
                *self.inner.state_watch_rx.borrow(),
            ...
            if self.inner.token.is_cancelled() {
                error!("Session has already been cancelled");
                return Err(StatusCode::BadInvalidState);
            }
        ...
        let channel_fut = self.inner.channel.run();
        pin!(channel_fut);

        select! {
            _ = self.inner.token.cancelled() => {
                info!("Session was cancelled");
            }
            r = &mut channel_fut => {
                return r;
            }
        ...
        // TODO: Disconnect self here

        let _ = self.inner.state_watch_tx.send(SessionState::Disconnected);

        select! {
            r = &mut channel_fut => r,
            _ = self.inner.channel.close_channel(true) => Ok(())
        }
    }

    async fn run_inner(&self) -> Result<(), StatusCode> {
        loop {
            self.ensure_and_activate_session().await?;
            let _ = self.inner.state_watch_tx.send(SessionState::Connected);

            let mut rx = self.inner.channel.get_state_change_rx();

            loop {
                if matches!(*rx.borrow(), State::Connected(_)) {
                    break;
                }
    ...
    async fn ensure_and_activate_session(&self) -> Result<(), StatusCode> {
        self.inner.channel.wait_for_connection().await;
